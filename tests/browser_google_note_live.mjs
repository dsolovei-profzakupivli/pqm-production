...
try{
 ...
 await evaluate(`(async()=>{showModule('edrMonitoring');document.querySelector('#edrMonitoringSearch').value='3485807842';await loadEdrMonitoring()})()`);
 const result=await evaluate(`(()=>{const button=document.querySelector('[data-edr-code="3485807842"] .edr-supplier-note');if(!button)return {visible:false};const row=button.closest('tr'),check=row.querySelector('.edr-monitoring-check');check.click();const selectedBefore=edrMonitoringSelected.has('3485807842');check.click();const deselectedAfter=!edrMonitoringSelected.has('3485807842');button.click();const dialog=document.querySelector('#googleNoteDialog');return{visible:true,inlineText:row.querySelector('[data-edr-column="google_note"]')?.textContent,selectedBefore,deselectedAfter,tooltip:button.title,dialogOpen:dialog.open,text:document.querySelector('#googleNoteText').textContent,readOnly:!dialog.querySelector('input,textarea,[contenteditable]')}})()`);
 if(!result.visible||!result.dialogOpen||!result.readOnly||result.text!=='Виключено, рішення АМКУ')throw new Error('Real snapshot Google note acceptance failed');
