...
(() => {
  ...
  const kinds={physical:'Колонка SQLite',json:'Поле JSON',derived:'Обчислюване поле'};
  ...
  ['schemaSearch','schemaGroup','schemaSource','schemaModule','schemaEditing','schemaStatus','schemaTechnical'].forEach(id=>q(id).addEventListener('input',draw));
  q('schemaReset').onclick=()=>{['schemaSearch','schemaGroup','schemaSource','schemaModule','schemaEditing','schemaStatus'].forEach(id=>q(id).value='');q('schemaTechnical').checked=false;draw()};
